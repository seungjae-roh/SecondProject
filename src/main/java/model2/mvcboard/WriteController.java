...
public class WriteController extends HttpServlet{
	...
	@Override
		...
		/*
		파일업로드를 위해 MultipartRequest객체를 생성한다. 객체가 정상적으로 생성된다면
		파일업로드는 완료된다.
		 */
		...
		디렉토리의 경로를 확인해본다.
		만약 글 작성시 파일을 첨부하지 않더라도 객체는 생성되므로 폼값은 정상적으로
		받을 수 있다.
		 */
		if (mr == null) {
			JSFunction.alertLocation(resp, "첨부 파일이 제한 용량을 초과합니다.", 
										"../mvcboard/write.do");
			/*
			객체가 생성되지 않으면 업로드에 실패하면 나머지 폼값도 받을 수 없으므로
			반드시 return을 통해 실행을 중지시켜야 한다.
			 */
			return;
		}
		//여기까지의 코드로 파일 업로드는 완료된다.
		...
		if (result == 1) {
			//insert에 성공하면 리스트로 돌아간다.
			resp.sendRedirect("../mvcboard/list.do?b_flag="+b_flag);
		}
		else {
			//실패하면 작성페이지로 다시 돌아간다.
