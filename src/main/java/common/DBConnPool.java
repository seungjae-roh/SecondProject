...
하는 API로 쉽게 말하면 외부에 있는 객체를 이름으로 찾아오기 위한 기술이다.
...
public class DBConnPool {
	...
	public DBConnPool() {
		try {
			...
			얻어온다. 여기서 톰캣의 루트 디렉토리명은 java:comp/env로 이미 
			정해져 있으므로 그대로 사용하면 된다.
			 */
			Context ctx = (Context)initCtx.lookup("java:comp/env");
			/*
			3. server.xml에 등록한 네이밍을 lookup하여 Datasource를
			얻어온다. 해당 데이터소스는 DB에 연결하기 위한 정보를 가지고 있다.
			 */
	...
	/*
	사용이 끝난 객체는 Pool에 반납한다. 즉, 여기서의 close()는 객체의
	소멸이 아닌 반납이다.
	 */
	public void close() {
		try {
