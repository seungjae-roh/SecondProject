...
/*
검색폼에서 입력한 검색어와 필드명을 파라미터로 받아온다. 해당 <form>의
전송방식은 get, action속성은 없는 상태이므로 현재 페이지로 폼값이 전송된다.
*/
...
가령 게시물의 갯수가 51개라면 나눴을 때 결과가 5.1이 된다. 이때 무조건
올림처리하여 6페이지가 나오게 한다.
만약 totalCount를 double로 형변환하지 않으면 정수의 결과가 나오게 되므로
6페이지가 아니라 5페이지가 되므로 주의해야 한다.
*/
...
List.jsp?pageNum => 이와같이 파라미터는 있는데 값이 없을 때는 빈값으로
	체크된다. 따라서 아래 if문은 2개의 조건을 구성해야 한다.
*/
int pageNum = 1;
...
/*
게시물의 구간을 계산한다.
MySQL에서는 limit을 통해 게시물의 범위를 결정하므로 Oracle을 사용할 때와
조금 다르게 구간을 계산해야 한다.
형식] 쿼리문 ... limit 시작위치, 한페이지에 출력할 갯수;
	...
	start =>
		...
		한페이지에 출력할 게시물의 갯수인 pageSize를 그대로 사용하면
		된다.
*/
int start = (pageNum - 1) * pageSize;

//한 페이지에 출력할 게시물의 갯수를 설정한다.
int end = pageSize;
...
<body>
	...
	<!-- 앞에서 계산해둔 전체 페이지 수와 파라미터를 통해 얻어온 현재
	페이지 번호를 출력한다. -->
	...
	<%
	    ...
	    for (BoardDTO dto : boardLists)
	    {
	        ...
	        //전체게시물 수 - (((현재페이지 - 1) * 한페이지 출력 갯수) + countNum증가)
